...
int client_send_message(Client* client, const uint8_t* payload, uint32_t length) {
    ...
    if (send_all(client->sock, &netlen, 4) == -1) return -1;
    if (send_all(client->sock, payload, length) == -1) return -1;
    return 0;
}

int client_recv_message(Client* client, uint8_t** out_buf, uint32_t* out_len) {
    uint32_t netlen = 0;
    if (recv_all(client->sock, &netlen, 4) == -1) return -1;
    uint32_t length = ntohl(netlen);
    if (length == 0 || length > MAX_MESSAGE_LENGTH) return -1;
    uint8_t* buf = (uint8_t*)malloc(length + 1);
    if (buf == NULL) return -1;
    if (recv_all(client->sock, buf, length) == -1) { free(buf); return -1; }
    buf[length] = '\0';
    *out_buf = buf;
    *out_len = length;
    return 0;
